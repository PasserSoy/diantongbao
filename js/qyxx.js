layui.use(['form','table','layer'],function(){
  ...
  // _msgTip(_url,'companyCountByApproveStatus');// 处理状态数据渲染
  // 表格
  jyTable = table.render({
    ...
    ,parseData: function(res){ //res 即为原始返回的数据
      _msgTip(res,'companyCountByApproveStatus');// 处理状态数据渲染
      if(res.errorCode.indexOf('please.login.first')>-1){// 监测登录过期
        ...
        return {
          ...
          "count": res.data.companytList.total, //解析数据长度
          "data": res.data.companytList.list //解析数据列表
        };
      };
    }
    ,cols:[[
      {type:'numbers',title:'序号',align:'center'},
      {field:'name',title:'企业名称',align:'center',minWidth:100},
      {title:'联系人',align:'center',minWidth:80,templet:function(d){return d.linkmanCountMap.qiYe}},
      ...
      {title:'状态',align:'center',width:110,
        templet:function(d){
          ...
          return `<span class="approveStatus approveStatus${d.approveStatus}" data-locsPro='${JSON.stringify(locsPro)}' data-href="qyxq.html?v=1.0.8">${myText}</span>`
        }
      }
    ]]
  });
